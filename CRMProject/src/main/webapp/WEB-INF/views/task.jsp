...
<%@taglib prefix="fn" uri="http://520it.com/java/crm" %>
...
<link rel="stylesheet" type="text/css" href="/js/fancy-box/jquery.fancybox.css?v=2.1.5" media="screen"/><!-- 小图变大图 -->
...
<body>
	...
	<div>
	 	<c:if test="${fn:checkPermission('com._520it.crm.web.controller.TaskController:save')}">
			<a  class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true"  data-cmd="add" >分配任务</a>
		</c:if>
	 	<c:if test="${fn:checkPermission('com._520it.crm.web.controller.TaskController:update')}">
			<a  class="easyui-linkbutton" data-options="iconCls:'icon-edit',plain:true"  data-cmd="eidte" >编辑任务</a>
		</c:if>
	 	<c:if test="${fn:checkPermission('com._520it.crm.web.controller.TaskController:del')}">
			<a  class="easyui-linkbutton" data-options="iconCls:'icon-cancel',plain:true"  data-cmd="del" >删除任务</a>
		</c:if>
	 	<c:if test="${fn:checkPermission('com._520it.crm.web.controller.TaskController:complate')}">
			<a  class="easyui-linkbutton" data-options="iconCls:'icon-ok',plain:true"  data-cmd=complete>标记完成</a>
		</c:if>
	 	<c:if test="${fn:checkPermission('com._520it.crm.web.controller.TaskController:lose')}">
			<a  class="easyui-linkbutton" data-options="iconCls:'icon-no',plain:true"  data-cmd="lose">标记失败</a>
		</c:if>
	 	<c:if test="${fn:checkPermission('com._520it.crm.web.controller.TaskController:addDescribe')}">
	...
	<div>
		<form id="searchbut">
			员工姓名查询:<input  type="text" name="keyword">
			 开始时间:
			<input class="easyui-datebox" name="start"/>结束时间:<input class="easyui-datebox" name="end"/>
			 <a  class="easyui-linkbutton" data-options="iconCls:'icon-undo'"  data-cmd="searchPage" >查询</a>
		 </form>
	</div>
	...
	<div id ="task_dialog">
				...
				<input  type="hidden"  name="id" >
						<table align="center"  style="margin-top: 40px;"  >
							...
							<tr align="center" >
									<td colspan="2">任务描述</td>
							</tr>
							<tr align="center">
									 <td colspan="2"><textarea  name="title"  style="height:150px;width: 200px;"></textarea></td>
							</tr>
						</table>
				</form>
	</div>
