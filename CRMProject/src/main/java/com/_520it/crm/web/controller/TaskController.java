...
public class TaskController {
    ...
    @RequestMapping("/task_change")
    @ResponseBody
    public AjaxResult change(Task todayTask) {
        AjaxResult result = null;
        try {
            this.taskService.change(todayTask);
            result = new AjaxResult(true, "更新成功");
        } catch (Exception e) {
            e.printStackTrace();
            result = new AjaxResult("发生异常了,请联系管理员!");
    ...
    @ResponseBody
    public AjaxResult addhandledescribe(Task task) {
        AjaxResult result = null;
        try {
    ...
    @RequestMapping("/task_complate")
    @ResponseBody
    public AjaxResult complate(Long cId) {
        AjaxResult result = null;
        try {
